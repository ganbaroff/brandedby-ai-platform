...
const Header = memo(function Header() {
  ...
  // Debug logging
  useEffect(() => {
    console.log('[Header] Mobile menu state:', mobileMenuOpen);
  }, [mobileMenuOpen]);
    ...
    >
      <div className="container mx-auto px-4 max-w-7xl">
        <div className="flex items-center justify-between h-16">
          ...
          <button 
            onClick={() => navigate('/')}
            className="flex items-center space-x-2 cursor-pointer group focus:outline-none focus:ring-2 focus:ring-primary-500 rounded-lg p-1"
            aria-label="BrandedBy - Go to homepage"
          >
            <div className="p-1.5 xs:p-2 bg-gradient-to-r from-blue-600 to-purple-600 rounded-lg group-hover:shadow-lg transition-all duration-300">
              <Sparkles className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
            </div>
            <div>
              <h1 className="text-base sm:text-lg md:text-xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                BrandedBY
              </h1>
              <p className="text-[10px] sm:text-xs text-gray-500 -mt-0.5">AI Video Generator</p>
            </div>
          </button>
          ...
          <nav className="hidden md:flex items-center space-x-4 lg:space-x-6 xl:space-x-8" id="navigation" role="navigation" aria-label="Main navigation">
            <button 
              ...
              className="text-sm lg:text-base text-gray-700 hover:text-primary-600 transition-colors duration-200 font-medium focus:outline-none focus:ring-2 focus:ring-primary-500 rounded px-2 py-1"
          ...
          <div className="hidden md:flex items-center space-x-2 lg:space-x-3">
            {user ? (
              ...
              <button
                onClick={handleAuthAction}
                className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-4 lg:px-6 py-2 rounded-xl font-semibold hover:shadow-lg hover:shadow-purple-500/25 transition-all duration-300 transform hover:scale-105 text-sm lg:text-base"
              >
                Sign In
              </button>
            )}
          ...
          <button
            onClick={() => {
              console.log('[Header] Menu button clicked, current state:', mobileMenuOpen);
              setMobileMenuOpen(!mobileMenuOpen);
            }}
            className="md:hidden p-2 text-gray-600 hover:text-purple-600 hover:bg-purple-50 focus:outline-none focus:ring-2 focus:ring-purple-500 rounded-xl transition-all duration-200 active:scale-95"
      ...
      >
        ...
        {/* Mobile Menu Panel */}
        <div 
          className={`absolute top-0 left-0 right-0 bg-white shadow-2xl overflow-y-auto transition-transform duration-300 ${
